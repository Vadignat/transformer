...
class TranslationDatasetProcessing():
    ...
    def __init__(self, cfg):
        ...
        train_dataset, test_dataset = train_test_split(self.dataset['train']['translation'], test_size=0.2, random_state=random_seed)
...
class TranslationDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch):
        ...
        padded_batch_lang_1 = [torch.tensor(sequence[0] + [self.translation_dataset_processing.tokenizers[self.cfg.languages[0]].token_to_id("<pad>")] * (
                    max_length_lang_1 - len(sequence[0])), dtype=int) for sequence in
                               batch]
        padded_batch_lang_2 = [torch.tensor(sequence[1] + [self.translation_dataset_processing.tokenizers[self.cfg.languages[1]].token_to_id("<pad>")] * (
